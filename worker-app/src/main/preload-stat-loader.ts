...
const sensorCache: Record<string, number> = {};
 ...
 */
function findNode(node: any | any[], type?: string, text?: string, imageURL?: string): any | null {
    if (!node) return null;
    ...
    for (const n of nodes) {
        ...
        if (imageURL && n.ImageURL === imageURL) return n;

        // Match by type + text if provided
        if (type && text && n.Type === type && n.Text === text) return n;
 ...
 */
function findNodeByID(node: any | any[], id: number): any | null {
    if (!node) return null;
...
// Polling helper
async function getSensorValue(key: string, type?: string, text?: string, imageURL?: string): Promise<string> {
